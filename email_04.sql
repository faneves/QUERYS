...
WITH MENSAL AS (
	...
	FROM (
		SELECT
		*,
		ROUND(PLANTA / CAST(SUM(PLANTA) OVER(ORDER BY CN ROWS BETWEEN UNBOUNDED PRECEDING AND UNBOUNDED FOLLOWING) AS float) * 100, 2) AS SHARE_PLANTA,
		ROUND(TEMPO / CAST(SUM(TEMPO) OVER(ORDER BY CN ROWS BETWEEN UNBOUNDED PRECEDING AND UNBOUNDED FOLLOWING) AS float) * 100, 2) AS SHARE_TEMPO,
...
'NO' AS REGIONAL
FROM MENSAL A
INNER JOIN SEMANAL B ON A.CN = B.CN
INNER JOIN DIARIO C ON A.CN = C.CN
INNER JOIN PROJECOES D ON A.CN = D.CN
ORDER BY A.CN