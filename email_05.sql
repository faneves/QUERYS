WITH MENSAL AS (
		...
		*,
		...
		FROM (
			...
			FROM (
				...
				WHERE YEAR(DATA_REFERENCIA) = 2024 AND MONTH(DATA_REFERENCIA) = 1 AND TECNOLOGIA <> 'GSM' AND B.NOME_REGIONAL = 'SPI' AND A.UF = 'SP'
		...
		*,
		...
		FROM (
			...
			FROM (
				...
				WHERE ANO = 2024 AND SEMANA = 3 AND TECNOLOGIA <> 'GSM' AND B.NOME_REGIONAL = 'SPI' AND A.UF = 'SP'
		...
		*,
		...
		FROM (
			...
			FROM (
				...
				FROM SISTEMA_MAESTRO..TBF_DISPONIBILIDADE_SETORES A WITH (NOLOCK)
				INNER JOIN INDICADORES_CORAN..TBA_DIVISAO_MUNICIPIOS_SP_FIXA B WITH (NOLOCK) ON A.MUNICIPIO = B.MUNICIPIO
				WHERE YEAR(DATA_REFERENCIA) = 2024 AND MONTH(DATA_REFERENCIA) = 1 AND DAY(DATA_REFERENCIA) >= 12 AND TECNOLOGIA <> 'GSM' AND B.NOME_REGIONAL = 'SPI' AND A.UF = 'SP'
	...
	FROM (
		...
		FROM SISTEMA_MAESTRO..AUX_SEMANAS A LEFT JOIN (
			...
			WHERE ANO = 2024 AND MES = 1 AND UF = 'INTERIOR'
